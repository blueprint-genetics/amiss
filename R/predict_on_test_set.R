...
#' @export
predict_on_test_set <- function(test_path, training_path, outcome_path, tr_output_path, results_dir_path, parameter_list, seed = 42) {
  ...
  flog.pid.info("PROGRESS Arguments: %s", paste0(list(test_path, training_path, outcome_path, tr_output_path, results_dir_path, seed), collapse = ", "))
  ...
  # Redo removal of duplicates arising from reduction of features
  training_data <- read.csv(normalizePath(training_path), row.names = 1)
  training_data <- training_data[, final_features]
  common_rows <- rbind(test_data, training_data)
  common_rows <- common_rows %>% duplicated(fromLast=TRUE)
  rows_to_keep <- !common_rows[1:NROW(test_data)]
