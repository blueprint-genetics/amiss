...
#'
#' @examples
cv_loop <- function(fold_tr_datas, fold_te_datas, fold_tr_outcomes, fold_te_outcomes, output_path, i) {
...
#' @return
#' @export
S11_cross_validation <- function(preprocessed_data_path, output_path, n_folds, seed = 10) {
  ...
  results_pair <- foreach::foreach(i = 1:length(folds), .packages = c("amiss", "purrr", "doRNG", "caret", "futile.logger", "magrittr", "DMwR2", "ModelMetrics")) %dopar% amiss::cv_loop(fold_tr_datas,fold_te_datas, fold_tr_outcomes, fold_te_outcomes, output_path, i)
