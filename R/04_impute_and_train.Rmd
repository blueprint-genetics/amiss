...
other_imputations <- lapply(enumerate(other_hyperparameter_grids), function(method) {
  ...
  if (method$name == "bpca") {
    ...
    return(imputation_object)
  }
  else {
    imputation_object <- lapply(1:nrow(hyperparams), function(x) {
      dats <- lapply(times, function(i) {
        result <- NULL
        method_function <- base::get(method$name)
        tryCatch({
          result <- do.call(method_function, c(list(quote(training_data)), x))
        }, error = function(e) {
          print("Trying to execute " %>% paste0(method$name, " the following error occurred: ", e$message))
        })
        return(result)
...
```{r}
imputations <- c(mice_imputations, other_imputations)
```
```{r}
...
select_best <- function(models, imputations, hyperparams, performance_function, positive) {
  ...
  # We can drop the now extraneous level "completed_datasets" of the tree
...
rf_bests <- select_best(rf_models, imputations, mice_hyperparameter_grids, performance_function = extract_oob_performance, TRUE)
lr_bests <- select_best(lr_models, imputations, mice_hyperparameter_grids, performance_function = extract_mcc_performance, FALSE)
```
