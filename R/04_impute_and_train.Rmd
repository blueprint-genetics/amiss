...
```{r}
deterministic_imputations <- lapply(enumerate(other_hyperparameter_grids), function(method) {
  ...
  if (method$name == "bpca") {
    imputations <- lapply(1:nrow(hyperparams), function(x) {
      run_bpca(data = training_data, hyperparams = hyperparams[x, ])
    })
  }
  else if (method$name == "knnImputation") {
    imputations <- lapply(1:nrow(hyperparams), function(x) {
      run_knn(data = training_data, hyperparams = hyperparams[x, ])
    })
  } else {
