...
names(deterministic_imputations) <- names(other_hyperparameter_grids)
```
...
single_value_imputations <- lapply(enumerate(single_value_imputation_hyperparameter_grids), function(method) {
  list(`1` = list(completed_datasets = list(get(method$name)(training_data))))
})
...
rf_models <- foreach(method = imputations) %do% {
  y <- foreach(mi_iter = method, .options.RNG = seed) %dorng% {
    z <- foreach(data = mi_iter$completed_datasets) %do% {
      return(train_rf(data))
    }
    names(z) <- names(mi_iter$completed_datasets)
    return(z)
  }
  names(y) <- names(method)
  return(y)
}
names(rf_models) <- names(imputations)
...
names(lr_models) <- names(imputations)
```
