...
impute_w_hps <- function(data, hp_tree){
  ...
  imputation_functions <- lapply(hp_tree, FUN = function(hps) {
    function(method) {
      ...
      } else if (method %in% single_value_imputation_hyperparameter_grids) {
  ...
  completions <- recursive_apply(imputations, median_imp, "data.frame")
...
turn_table <- function(perf_tree) {
  ...
  perf_df[["X4"]] <- NULL
  colnames(perf_df) <- c("method", "model_index", "test_realization", "training_realization", "value")
  return(perf_df)
}
...
all_table <- Reduce(function(x, y) merge(x, y, by = c("method", "model_index", "test_realization", "training_realization")), tables)
colnames(all_table) <- c("method", "model_index", "test_realization", "training_realization", names(tables))
all_table
```
