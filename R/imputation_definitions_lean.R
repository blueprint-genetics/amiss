...
mice_hyperparameter_grids <- lapply(mice_imputation_hyperparameters, expand.grid) %>% lapply( .%>% head(2))
...
other_hyperparameter_grids <- lapply(deterministic_imputation_hyperparameters, expand.grid) %>% lapply( .%>% head(2))
