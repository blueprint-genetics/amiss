...
for (i in seq_along(child_run_ids)){
  ...
  ## Get XGBoost results
  download_file_from_run(child_run_iter, "outputs/cv_xg_results.csv")
  xg_results <- read.csv("cv_xg_results.csv") %>% mutate(source = "xg_result")
  file.remove("cv_xg_results.csv")
  
  ## Append to dataframe
  iter_results <- rbind(lr_results, rf_results, xg_results)
